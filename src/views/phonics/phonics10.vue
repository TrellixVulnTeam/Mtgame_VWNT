              ...
              <div class="listbar">
                ...
                <div v-bind:class="{'answer':answer}" class="leftimg" @click="change">
                 <!-- <img :src=images style="width:80%; height:80%; position: absolute;left: 9%;top: 10%;border-radius: 50px;"> -->
                  <img src="../../assets/image/speat.png" style="width: 100%; float: left;">
                </div>
                <div class="listbar2">

                  <div>
                    <button v-for="item in items2" :key="item.id" class="answer-items" v-bind:class="{liopciay:item.liopciay}" @click="onkey(item)">
                      <p class="answeritems">{{item.name}}</p>
                    </button>
                  </div>
                  <div>
                    <button v-for="item in items3" :key="item.id" class="answer-items"  v-bind:class="{liopciay:item.liopciay}" @click="onkey(item)">
                      <p class="answeritems">{{item.name}}</p>
                    </button>
                  </div>
                  <div>
                    <button v-for="item in items4" :key="item.id" class="answer-items"  v-bind:class="{liopciay:item.liopciay}"  @click="onkey(item)">
                      <p class="answeritems">{{item.name}}</p>
                    </button>
                  </div>
                </div>
  ...
  export default {
        ...
        if (this.onef == true) {
          setTimeout(() => {
            ...
            for(var i=0;i<this.items2.length;i++){
              this.items2[i].liopciay=false;
            }
            for(var i=0;i<this.items3.length;i++){
              this.items3[i].liopciay=false;
            }
            for(var i=0;i<this.items4.length;i++){
              this.items4[i].liopciay=false;
            }
            for(var i=0;i<this.listout.length;i++){
              this.listout[i].bgc=false;
            }
            this.timeout=0;
            this.onef = false;
            // this.video = true;
    ...
    data() {
      return {
        ...
        warn: "Listen to the word and write the missing letter.",
...
<style scoped lang="less">
  ...
  .phonics10 {
    width: 100%;
    height: 100%;
    background-image: url("http://107.150.121.34/monkeytownHK/image/img/phonicsbg2.png");
